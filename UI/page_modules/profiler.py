...
def show_profiler(result):
    ...
    # Profiler sub-tabs
    profiler_tabs = st.tabs(["Overview", "Financials", "Geographic Mix", "Management", "Recent News", "M&A Profile"])
    ...
    with profiler_tabs[5]:
        ma_str = "This section could include profiles or key information about the mergers and acquisitions team."
        st.write("### M&A Profile")
        st.write(ma_str)
        agree = st.checkbox("Add M&A Profile to presentation",
                            value=st.session_state.get('M&A Profile', False))

        st.session_state["M&A Profile"] = ma_str if agree else False
